...
'cc2node ' + pkg.version + ' — Bun-compiled Claude Code → pure Node\n' +
'\n' +
'Usage:\n' +
'  cc2node <version|tarball|binary> [options]\n' +
'\n' +
...
function parseArgs(argv) {
  var a = { _: [], platform: null, out: null, targets: null, transpile: true, ripgrep: true, install: true, keepTemp: false, help: false, version: false };
  for (var i = 0; i < argv.length; i++) {
    var x = argv[i];
    switch (x) {
      case '-h': case '--help': a.help = true; break;
      case '-v': case '--version': a.version = true; break;
      case '-p': case '--platform': a.platform = argv[++i]; break;
      case '-o': case '--out': a.out = argv[++i]; break;
      case '-t': case '--targets': a.targets = argv[++i]; break;
      case '--no-transpile': a.transpile = false; break;
      case '--no-ripgrep': a.ripgrep = false; break;
      case '--no-install': a.install = false; break;
      case '--keep-temp': a.keepTemp = true; break;
      default:
        ...
        else if (x[0] === '-') { log.err('unknown option: ' + x); process.exit(2); }
        else a._.push(x);
    }
  }
  return a;
...
(function main() {
  ...
  if (args.help || !args._.length) { help(); process.exit(args.help ? 0 : 1); }
  ...
  var targets = args.targets ? args.targets.split(',').map(function (s) { return s.trim(); }).filter(Boolean) : null;
